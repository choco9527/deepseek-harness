...
此包需要会话存储、会话持久化后端，以及保存其记录的存储行。配置可省略。最小组合如下：
...
可选配置 `pathRelocations` 接受由宿主在启动前准备的 `{ from, to }` 目录对。两个完全限定路径必须通过保留的文件系统别名解析到同一现存目录。注册表在索引会话头之前更新匹配记录（含子目录）；ID、顺序、归档状态和会话日志保持不变。重复来源、无效目录和目标路径冲突会在迁移写入前拒绝激活。进程互斥、文件复制和别名由宿主负责；注册表不自动发现或移动目录。参见[迁移决策](../../../.agents/notes/implemented/architecture/2026-09-18-workspace-path-relocation.zh.md)与[配置目录](../../../docs/config-catalog.zh.md)。
