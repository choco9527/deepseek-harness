...
Plugin context travels in the user prompt's durable inbox source. The `agent/pre-step` listener expands it into plugin-source messages immediately before the accepted user message, preserving their shared request identity. Queue edits preserve the context; removal discards it with the prompt; steering moves both together. Replaying pending inbox events restores the same ownership without a process-local context cache. Context-only prompts retain an empty user message for request correlation and annotation display.
...
### Submitted plugin context

#### What the model sees

Each submitted context contributes its exact `text` as a plugin-source user message immediately before its accepted prompt. Queued context never enters an earlier turn. The source records `plugin`, and annotation context also records `form` and the prompt's `submissionId`.

#### Token effect

Proportional to the submitted context text.

#### KV Cache effect

Context appends with its prompt and does not rewrite earlier model history.
