...
- **Health does not execute plugins** — discovery checks named rows against installed packages, existing files, or the host's parent-aware CommonJS resolver. ESM-only hooks without a matching CommonJS resolver remain unsupported. A package present on disk with a missing entry, a plugin that throws on apply, or one waiting for a service can still fail at mount. Rows with a `disabled` expression remain unchecked until the Loader evaluates them.
