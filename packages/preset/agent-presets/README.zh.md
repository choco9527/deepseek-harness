...
- **健康检查不执行插件**——发现过程通过已安装的包、存在的文件或宿主支持父路径的 CommonJS 解析器检查具名行。仅有 ESM 钩子而没有对应 CommonJS 解析器的宿主仍不受支持。磁盘上存在但入口缺失的包、在 apply 时抛错的插件、等待服务的插件仍可能在挂载时失败。带有 `disabled` 表达式的行仍留待 Loader 求值后检查。
