...
function pluginSourceValue(source: JsonRecord, label: string): void {
  ...
  if (source['form'] === 'annotation') optional.push('submissionId')
  ...
  literalValue(form, ['instructions', 'catalog', 'snapshot', 'notice', 'relay', 'recall', 'annotation'], `${label} form`)
  if (form === 'annotation') nonEmptyString(source['submissionId'], `${label} submissionId`)
  if (form === 'snapshot') {
