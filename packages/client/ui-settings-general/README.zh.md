...
`dsh-client-ui-settings-general` 是 dsh Web 客户端的设置外壳：Settings 以弹窗或全窗口页面从侧边栏底部的控件打开，该控件旁的连接故障指示器提供即时恢复操作；导航由各功能贡献的分区构建；首次运行的用户一次只走一个引导步骤。它还注册设置页面上所有不属于单一功能的内容：触发器、页面标题与返回控件界面框架、「本地配置文件」操作、「通用」分区及其 `settings.general.item` slot，以及 `settings` 字典。归具体功能所有的行（「权限」、「语言」、「外观」）、分区（「模型」）与条件式首次使用引导步骤仍由各自的功能包提供；外壳本身不自带任何引导文案。
...
用户通过侧边栏底部的 Settings 控件进入外壳；功能插件通过本外壳所投影的 slot 账本贡献自己的页面与引导步骤。Host 的 `presentation` 配置通过 `ui-settings-shell` 设置基础值选择 `modal`（默认）或 `page`；显式保存的字段覆盖基础值。两种展示共享导航、分区 slot 和打开状态。弹窗通过关闭操作、遮罩或 Esc 关闭。全窗口页面提供**返回应用**与 Esc，点击背景不会关闭。两种展示关闭后均将焦点还给 Settings 入口。Host 连接失败后，浅黄色的**连接异常**操作会出现在 Settings 右侧；自动恢复期间显示**连接中**，其后一至三个点每 500ms 前进一次。鼠标悬浮或键盘聚焦任一黄色状态时，只有文案变为**立即重连**，背景保持不变；按压反馈留在黄色色阶内，选中后立即从 retry 1 开始。恢复后该区域变为浅绿色的**连接成功**，驻留 2 秒再消失。所有可见状态的文字都左对齐，且图标、文字起点、高度和宽度保持固定。首次启动与未曾中断的健康连接保持静默。外壳还会每次只挂载一个引导步骤。
...
宿主端在用户设置 seam 中注册 `ui-settings-shell`，提供设置布局的基础值，并注册 `ui-onboarding`。`ui-settings-models` 提供的欢迎步骤通过既有公开 settings 边界读写其中的 `welcomeNoticeVersion`；外壳本身仍不持有产品策略。
