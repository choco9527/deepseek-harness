...
Users reach the shell through the sidebar's bottom Settings control; feature plugins contribute their pages and onboarding steps through the slot ledgers this shell projects. The Host `presentation` config selects `modal` (the default) or `page` through the `ui-settings-shell` settings base; explicit saved fields override the base. Both presentations share the same navigation, section slots and open state. The modal closes through its close action, mask or Escape. The full-window page provides **Back to app** and Escape, without background dismissal. Both restore focus to the Settings trigger on close. After a Host connection failure, a pale-yellow **Disconnected** action appears to the right of Settings. Automatic recovery shows **Connecting** with one to three dots advancing every 500ms. Hover or keyboard focus changes either yellow label to **Reconnect now** without changing its background; press feedback stays within the warning palette, and selecting it starts retry 1 immediately. Recovery changes the region to pale-green **Connected** for two seconds before it disappears. The icon, left-aligned text origin, height, and width remain fixed across every visible state. Initial startup and uninterrupted healthy operation remain silent. The shell also mounts exactly one onboarding step at a time.
...
The Host half registers `ui-settings-shell` with presentation defaults and `ui-onboarding` in the user-settings seam. The welcome step contributed by ui-settings-models reads and writes its `welcomeNoticeVersion` through the existing public settings boundary; the shell itself remains policy-free.
