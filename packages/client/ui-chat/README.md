...
Applications may set `ui-chat.config.toolsOnlyTranscript: true` to keep Assistant messages, images, context, and retries in their original positions while folding only Tool rows in eligible completed Turns. The control counts tools and subagent calls, never messages; prose-only Turns have no control, and inline reasoning retains its independent disclosure. This fixed policy hides the conversation-display selector and ignores stored or arriving user mode preferences without rewriting them. Open Turns, incomplete history, and closed Turns without a final answer remain expanded. Omitted or false configuration preserves the preference-driven behavior below.
