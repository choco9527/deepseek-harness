...
应用可设置 `ui-chat.config.toolsOnlyTranscript: true`，让 Assistant 消息、图片、上下文和重试保持原位可见，仅收起符合条件的已完成轮次中的工具行。控件只统计工具调用和 subagent 委派，不统计消息；纯正文轮次不显示控件，行内推理保留独立展开入口。该固定策略隐藏对话显示选择器，忽略已保存或随后到达的用户模式偏好，但不改写这些偏好。进行中的轮次、不完整历史及没有最终正文的已关闭轮次仍保持展开。不配置或设为 false 时保留下述由用户偏好控制的行为。
