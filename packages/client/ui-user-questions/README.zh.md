...
当 agent 在 Web 客户端中提问时，本包会用交互式提问界面接管聊天编辑器。用户可以在问题之间导航、选择一个或多个选项、输入自定义答案、跳过问题，并提交一批结构化答案。单选项点击后立即前进，最后一题提供行内提交按钮，而草稿会在当前页面的生命周期内跨 Session 导航保留。若唯一的问题声明了受支持的呈现意图，则可使用专用界面，包括带 `Chat about it`、`Refuse` 和 `Approve` 操作的 plan-review 卡片。
...
当 agent 提问时，回答每个问题、用翻页器导航，或跳过它。点击单选项后立即前进，最后一题则只选中答案。悬浮或聚焦最后一题的单选项时，会显示带底色的小「提交」按钮；点击后选择该项并提交整组答案，或返回尚未回答的问题。触摸设备始终显示这些按钮。多选和自定义答案保留底部操作。Enter 继续流程，并在所有问题均已回答或跳过后提交；Shift+Enter 换行，IME 组合输入期间的 Enter 只确认输入候选。
