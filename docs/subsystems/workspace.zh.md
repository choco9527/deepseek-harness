...
启动配置接受 `pathRelocations: WorkspacePathRelocation[]`；[配置目录](../config-catalog.zh.md#deepseek-aidsh-workspace)负责这两个类型声明。宿主必须在激活前完成物理移动并保留别名。[workspace 包](../../packages/workspace/workspace/README.zh.md)定义校验与元数据保留保证。
