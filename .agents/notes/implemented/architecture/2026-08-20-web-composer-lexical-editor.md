...
- **Chips are atomic `DecoratorNode`s** (`ReferenceChipNode`) carrying the owner's insert-time projections. NodeKey is the editor identity; the shell publishes an `occurrenceId` for each live node so input extensions can call `SessionInput.removeReference()` without touching Lexical. `getTextContent()` answers the clipboard projection, so native copy/cut and the draft mirror need no expansion code.
...
- `InputState.draft` is the clipboard projection (was display text). Input extensions may read the runtime-only occurrence table and remove a live chip by `occurrenceId`; ids never cross a shell or persistence boundary.
- Chip deletion follows the engine's native decorator gesture or `SessionInput.removeReference(occurrenceId)`; jsdom lacks `Selection.modify`, so the keyboard path is asserted in the browser lane only.
