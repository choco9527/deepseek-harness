...
Pi-ai Chat Completions 路由还可通过 `maxRequestBodyBytes` 限制完整 UTF-8 JSON 请求。此可选检查在 HTTP 发送前校验 SDK 载荷，包含工具和文本。其图片投影预留最新用户附件及该用户之后最新一条含图片消息，再卸载更早的图片。预估阶段不会在请求版本准备前拒绝受保护图片；精确图片大小或完整请求体超限时返回 `INVALID_REQUEST`，不重试。它不改写会话、不删除附件。其他协议在最终载荷钩子完成验证前拒绝此选项。
...
**依靠 token 压缩满足 HTTP 字节上限。** Base64 图片可能在 token 窗口达到压缩阈值前超过网关请求体上限。独立字节预算避免这类不匹配；文本或工具仍然超限时，需要明确缩小请求，而非隐藏重试或有损摘要。
